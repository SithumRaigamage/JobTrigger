import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:job_trigger/presentation/features/home/job_search_notifier.dart';

void main() {
  test('initial state is an empty query', () {
    final container = ProviderContainer();
    addTearDown(container.dispose);

    expect(container.read(jobSearchNotifierProvider), '');
  });

  test('setQuery updates the state to the given query', () {
    final container = ProviderContainer();
    addTearDown(container.dispose);

    container.read(jobSearchNotifierProvider.notifier).setQuery('ci');

    expect(container.read(jobSearchNotifierProvider), 'ci');
  });

  test('clear resets the state to an empty query', () {
    final container = ProviderContainer();
    addTearDown(container.dispose);
    container.read(jobSearchNotifierProvider.notifier).setQuery('ci');

    container.read(jobSearchNotifierProvider.notifier).clear();

    expect(container.read(jobSearchNotifierProvider), '');
  });
}
